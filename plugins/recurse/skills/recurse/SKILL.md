...
Declare `inputs.properties.task` as a string that is required from the caller or has a non-empty default. Its resolved value becomes the agent task. Inside tools, `recurse.context().inputs` is a read-only mapping of other resolved caller inputs; `task` is excluded. Write artifact files under `recurse.context().workspace`.
...
Runs continue while the account funds them, up to the provider's fixed 24-hour limit with three minutes reserved for finalization. Direct runs are preemptible by default; add `--non-preemptible` to avoid provider Function preemption at 3× the Function CPU and memory rate. Other charges keep their normal rates. Use `recurse status <run-id>` to inspect a run or `recurse cancel <run-id>` to request cancellation. Download available artifacts within 24 hours using `recurse artifacts <run-id> --output results`.

Connect an MCP host via `recurse mcp serve <deployment-id>`. For Codex, set `startup_timeout_sec = 180` and `tool_timeout_sec = 87000` in the server configuration so a long run is not cut short by the host. See the [deployment guide](https://recurse.run/docs/deploy).
...
| --- | --- |
| `0` | Confirmed terminal run, including `failed`, `cancelled`, `timed_out`, or `preempted`; inspect YAML `status`. |
| `1` | Handled CLI or API error; inspect YAML `error` and any run ID before retrying. |
| `130` | CLI interrupted by Ctrl-C; inspect YAML and the reported remote state. |

`recurse status <run-id>` writes the same version-1 YAML snapshot and exits `0` for every retrieved state, including unsuccessful runs. The Engine can report `preempted` when it detects provider interruption; detection is best effort and does not start a replacement run. Handled CLI errors write YAML with `error.type: cli` and exit `1`; preserve any run ID and inspect the remote state before retrying.
...
| `execution_failed`, `infrastructure_failed` | Retain the run ID and available evidence; do not invent a cause. An infrastructure failure has public `status: failed` and `error.code: infrastructure_failed`. |

`authentication_failed` directs you to `recurse login`. `request_failed` does not establish that remote execution stopped. If state is unknown, execution and charges may continue: inspect the existing run before rerunning. Keep the admission reference if no run ID was obtained.
...
`status` reports the current state and output availability in the YAML snapshot. Inspect again
after requesting cancellation to see the terminal state. Download artifacts while
`outputs.availability` is `available`, within 24 hours after completion. `artifacts: []` means
a finalized empty inventory; `artifacts: null` means the inventory is pending or unavailable.
Artifact retrieval verifies size and SHA-256 before atomically replacing an existing regular file;
it rejects unsafe paths and destination directories. After expiry, `status` retains artifact
metadata and the original deadline, but result content and downloads are unavailable. After
failure, inspect preserved evidence before deciding whether a new run is useful.
...
For Codex, set `startup_timeout_sec = 180` and `tool_timeout_sec = 87000` in that server's
